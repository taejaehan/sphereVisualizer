...
var Sphere = function(callback) {
  ...
  var btnColor = new THREE.Color("rgb("+r+", "+g+", "+b+")");

  var geometry = new THREE.ConeGeometry( 1, 2, 3 );
  var material = new THREE.MeshBasicMaterial( { color: btnColor, side: THREE.DoubleSide});
  var play = new THREE.Mesh( geometry, material );
 
  var geo = new THREE.EdgesGeometry( geometry );
  var mate = new THREE.LineBasicMaterial( { color: "black", linewidth: 2 } );
  var wireframe = new THREE.LineSegments( geo, mate );
  play.add(wireframe)
  play.rotation.z = -(Math.PI / 2);
  scene.add( play );
  camera.position.z = 5;
  renderer.domElement.addEventListener("click", callback, true);
  ...
  this.changeStatus = function(st){
    renderer.domElement.removeEventListener("click", callback, true);
    scene.remove( play );
    scene.add( sphere );
    STATUS = st;
  }
  this.setAnalyser = function(obj){
    ANALYSER = obj
  }
  this.loop = function animate() {
    ...
    if(STATUS == 'INIT'){
      ...
      play.rotation.x += 0.01;
      // play.rotation.y += 0.01;
    }else{
